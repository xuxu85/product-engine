...
This adapter runs the official BrowserAct Amazon Product Search workflow and
returns provider observations only. It does not make business decisions.
"""
...
def run_search(
    ...
    """Run one BrowserAct Amazon Product Search task and return its output."""
    ...
    template_id = template_id or os.getenv(
        "BROWSERACT_PRODUCT_SEARCH_TEMPLATE_ID", DEFAULT_TEMPLATE_ID
    )
    payload = {
    ...
    while time.monotonic() < deadline:
        status = _request(
            f"{API_BASE}/get-task-status?{urllib.parse.urlencode({'task_id': task_id})}",
            api_key,
        )
